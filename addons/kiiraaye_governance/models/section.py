...
class KiiraayeSection(models.Model):
    ...
    def _onchange_country_and_type(self):
        ...
        # Un changement de type ou de pays remet à zéro la chaîne géographique.
        ...
        if self.region_id and self.region_id.country_id != self.country_id:
            self.region_id = False
        if self.departement_id and self.departement_id.country_id != self.country_id:
            self.departement_id = False
        if self.commune_id and self.commune_id.country_id != self.country_id:
            self.commune_id = False
        if self.quartier_id and self.quartier_id.country_id != self.country_id:
            self.quartier_id = False
        ...
        return {
            "domain": {
                ...
                "quartier_id": [
                    ("country_id", "=", self.country_id.id),
                    ("niveau", "=", "niveau5"),
                    ("parent_id", "child_of", self.commune_id.id) if self.commune_id else ("id", "=", False),
                    ("active", "=", True),
                ],
            }
        }
    ...
    def _onchange_commune_id(self):
        if self.quartier_id and self.quartier_id.parent_id and self.commune_id:
            ...
            valid = False
            while ancestors:
                if ancestors.parent_id == self.commune_id:
                    valid = True
                    break
    ...
    def _check_geography(self):
        for record in self:
            if record.region_id:
                if record.region_id.country_id != record.country_id or record.region_id.niveau != "niveau1":
                    raise ValidationError(
                        _("La région doit appartenir au pays sélectionné et être de niveau Région.")
                    )

            if record.departement_id:
                if (
                    record.departement_id.country_id != record.country_id
                    or record.departement_id.niveau != "niveau2"
                    or (record.region_id and record.departement_id.parent_id != record.region_id)
                ):
                    raise ValidationError(
                        _("Le département doit appartenir à la région et au pays sélectionnés.")
                    )

            if record.commune_id:
                if (
                    record.commune_id.country_id != record.country_id
                    or record.commune_id.niveau != "niveau3"
                    or (record.departement_id and record.commune_id.parent_id != record.departement_id)
                ):
                    raise ValidationError(
                        _("La commune doit appartenir au département et au pays sélectionnés.")
                    )

            if record.quartier_id:
                if record.quartier_id.country_id != record.country_id or record.quartier_id.niveau != "niveau5":
                    raise ValidationError(
                        _("Le quartier doit appartenir au pays sélectionné et être de niveau Quartier.")
                    )
                if record.commune_id:
                    current = record.quartier_id.parent_id
                    valid = False
                    while current:
                        if current == record.commune_id:
                            valid = True
                            break
            ...
            if record.type_section == "regionale" and not record.region_id:
                raise ValidationError(_("Une coordination régionale doit avoir une région."))

            if record.type_section == "departementale":
                if not record.region_id or not record.departement_id:
                    raise ValidationError(
                        _("Une coordination départementale doit avoir une région et un département.")
                    )

            if record.type_section == "communale":
                if not record.region_id or not record.departement_id or not record.commune_id or not record.quartier_id:
                    raise ValidationError(
                        _("Une section communale doit avoir une région, un département, une commune et un quartier.")
                    )
    ...
    def _check_unique_geography_scope(self):
        for record in self:
            ...
            domain = [
                ("id", "!=", record.id),
                ("type_section", "=", record.type_section),
                ("country_id", "=", record.country_id.id),
            ]
            if record.type_section == "regionale":
                domain += [("region_id", "=", record.region_id.id)]
            elif record.type_section == "departementale":
                domain += [
                    ("region_id", "=", record.region_id.id),
                    ("departement_id", "=", record.departement_id.id),
                ]
            elif record.type_section == "communale":
                domain += [
                    ("region_id", "=", record.region_id.id),
                    ("departement_id", "=", record.departement_id.id),
                    ("commune_id", "=", record.commune_id.id),
                    ("quartier_id", "=", record.quartier_id.id),
                ]
            elif record.type_section in ("nationale", "diaspora"):
                domain += [
                    ("region_id", "=", False),
            ...
            if duplicate:
                raise ValidationError(
                    _("Une section/coordination de ce type existe déjà pour cette localisation géographique.")
                )
    ...
    def create(self, vals_list):
        for vals in vals_list:
            ...
            if type_section in ("nationale", "diaspora"):
                vals.update(
                    {
                        "region_id": False,
                        "departement_id": False,
