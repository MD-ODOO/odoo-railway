...
class KiiraayeResultat(models.Model):
    _name='kiiraaye.resultat'; _description='Résultat géographique agrégé'
    campagne_id=fields.Many2one('kiiraaye.campagne',required=True,ondelete='cascade',index=True); geographie_id=fields.Many2one('kiiraaye.geographie',required=True,ondelete='restrict',index=True)
    ...
    _sql_constraints=[('uniq','unique(campagne_id,geographie_id)','Résultat déjà saisi pour cette campagne et cette zone géographique.')]
