...
export class KiiraayeTerritoryDashboard extends Component {
 ...
 async load(){
  ...
  const [t,s,b,m,a,f,ch]=await Promise.all([
   this.orm.read('kiiraaye.territoire',[id],['name','complete_name','niveau','latitude','longitude']),
   ...
   this.orm.searchRead('kiiraaye.territoire',[['parent_id','=',id]],['name','niveau','partisan_count','section_count'],{order:'name'})
  ]);
