...
CMD ["sh", "-c", "chown -R odoo:odoo /var/lib/odoo && exec su -s /bin/bash odoo -c 'odoo -u kiiraaye_governance --db_host=\"${ODOO_DB_HOST:-postgres.railway.internal}\" --db_port=\"${ODOO_DB_PORT:-5432}\" --db_user=\"${ODOO_DB_USER:-odoo}\" --db_password=\"${ODOO_DB_PASSWORD:-}\"'"]
